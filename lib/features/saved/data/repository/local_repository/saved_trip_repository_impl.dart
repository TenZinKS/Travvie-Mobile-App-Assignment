...
class SavedTripRepositoryImpl implements SavedTripRepository {
  final LocalSavedTripDataSource localDataSource;

  SavedTripRepositoryImpl(this.localDataSource);

  @override
  Future<Either<Failure, void>> addSavedTrip(SavedTripEntity trip) async {
    try {
      await localDataSource.addSavedTrip(SavedTripModel.fromEntity(trip));
  ...
  @override
  Future<Either<Failure, void>> deleteSavedTrip(String tripId) async {
    try {
      await localDataSource.deleteSavedTrip(tripId);
      return const Right(null);
    } catch (e) {
      return Left(LocalDatabaseFailure(message: e.toString()));
    }
  ...
  Future<Either<Failure, List<SavedTripEntity>>> getAllSavedTrips() async {
    try {
      final models = await localDataSource.getAllSavedTrips();
